...
class TestClientAPI:
    ...
    def test_clients_get(self, client, client_1, client_2):
        ...
        assert isinstance(
            test_data, list
        ), "Check that a GET request to `/api/v1/clients/` returns a list"
    ...
    def test_client_create(self, client, db, client_1, client_2, tag_1):
        ...
        test_data = response.json()
        msg_error = (
            "Check that a POST request to `/api/v1/clients/` "
        ...
        data["tags"].append(
            data["phone_number"][1:4]
        )  # add mobile_operator_code to tags

        assert sorted(test_data.get("tags")) == sorted(data["tags"]), msg_error

        assert (
            clients_count + 1 == Client.objects.count()
        ), "Check that a POST request to `/api/v1/tags/` creates a client"

    def test_client_does_not_create_with_not_7_code(
        self, client, db, client_1, client_2, tag_1
    ):
        data = {
    ...
    def test_client_does_not_create_with_not_right_format_phone_number(
        self, client, db, client_1, client_2, tag_1
    ):
        data = {
