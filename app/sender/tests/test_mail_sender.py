...
class TestMailAPI:
    ...
    def test_mail_sender_get(self, client, db, mail_sender_1):
        ...
        assert test_mail_sender['sending_start'] == mail_sender.sending_start.strftime(time_format)
        assert test_mail_sender['sending_stop'] == mail_sender.sending_stop.strftime(time_format)
        assert test_mail_sender['text'] == mail_sender.text
        assert test_mail_sender['filters'] == list(mail_sender.filters.values_list('name', flat=True))
    ...
    def test_mail_sender_create(self, client, db, mail_sender_1, tag_1):
        ...
        response = client.post('/api/v1/campaigns/', data=data, format='json', content_type="application/json")
        print(response.json())
